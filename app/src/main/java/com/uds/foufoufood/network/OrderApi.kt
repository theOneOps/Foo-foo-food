...
interface OrderApi {
    ...
    suspend fun createOrder(
        @Header("Authorization") token: String, @Body orderRequest: OrderRequest
    ): Response<OrderResponse>

    @GET("api/orders/hasActiveOrder/{clientEmail}")
    suspend fun hasActiveOrder(
        @Header("Authorization") token: String, @Path("clientEmail") clientEmail: String
    ...
    suspend fun updateOrderStatus(
        @Path("id") orderId: String, @Body statusRequest: OrderStatusUpdateRequest
    ): Response<Order>
